...
/**
 * LLMServiceTest is a simple verification runner that confirms all four locally
 * hosted LLMs are reachable and responding correctly via LLMService.
 *
 * This is not a unit test framework class (no JUnit). It is a standalone Java
 * application intended to be run manually in Eclipse to verify the Ollama
 * server is running and all models are loaded correctly before beginning the
 * nutritional analysis experiments.
 *
 * How to run: 1. Ensure Ollama is running (look for the Ollama icon in the menu
 * bar) 2. Right-click this file in Eclipse -> Run As -> Java Application 3.
 * Check the Console tab for responses from each model
 *
 * Expected output: Each model should return a short response to the test
 * prompt. If a model fails, an error message is printed and the next model is
 * tried. 11
 */
public class LLMServiceTest {
	...
	public static void main(String[] args) {
		...
		String[] models = { LLMService.LLAMA, // Meta Llama 3.2 3B
		...
		for (String model : models) {
			...
			try {
				...
				String response = service.prompt(model, "You are a helpful assistant.", testPrompt);
